            ...
            <div className="alerts-table-container">
              ...
              {medications.map((medication) => (
                <div className="alerts-table-row" key={medication.medicamentoid}>
                  ...
                  <div className="column-name">{medication.nomemedicamento}</div>
                  <div className="column-description">{medication.descricao}</div>
                  <div className="column-available">{medication.quantidadedisponivel}</div>
                  <div className="column-minimum">{medication.quantidademinima}</div>
                </div>
              ))}
            </div>
          ) : (
              ...
              {orders.map((order) => (
                <div className="alerts-table-row" key={order.encomendaid}>
                  ...
                  <div className="column-minimum">
                    {order.dataentrega ? new Date(order.dataentrega).toLocaleDateString() : 'N/A'}
                  </div>
                </div>
              ))}
            </div>
          ...
          {requests.length > 0 ? (
            <div className="alerts-table-container">
              <div className="alerts-table-header">
                ...
                <div className="column-name">First Name</div>
                <div className="column-description">Last Name</div>
                <div className="column-available">Request Date</div>
                <div className="column-minimum">Delivery Date</div>
              </div>
              {requests.map((request) => (
                <div className="alerts-table-row" key={request.requisicaoid}>
                  <div className="column-id">{request.requisicaoid}</div>
                  <div className="column-name">{request.nomeproprio}</div>
                  <div className="column-description">{request.ultimonome}</div>
                  <div className="column-available">
                    {new Date(request.datarequisicao).toLocaleDateString()}
                  </div>
                  <div className="column-minimum">
                    {request.dataentrega ? new Date(request.dataentrega).toLocaleDateString() : 'N/A'}
                  </div>
                </div>
              ))}
            </div>

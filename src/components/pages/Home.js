...
function Home() {
  ...
  useEffect(() => {
    ...
    fetch(`${baseURL}/api/checkDatabase/check`)  // Calls the /api/checkDatabase endpoint
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => setMedicamentos(data))  // Updates the state with the fetched data
      .catch(error => {
        console.error('Error fetching medicamentos:', error);
      });

    // Fetch Alertas (Stock Alerts) data using checkDatabase (you can modify the query to match your requirements)
    fetch(`${baseURL}/api/checkDatabase/check`)  // Calls the /api/checkDatabase endpoint
      .then(response => response.json())
      .then(data => setAlertas(data))  // Updates the alertas state with the fetched data
